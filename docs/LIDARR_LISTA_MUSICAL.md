...
- Reguetón (sí reggae jamaiquino clásico).
- Grupera, regional mexicano y banda sinaloense/comercial.
...
Este documento es una **lista de referencia**, no un import list automático. Para agregar los artistas:

1. En Lidarr, ve a **Artists → Add New Artist** y busca cada nombre de la columna "Artista" (Lidarr los resuelve contra MusicBrainz).
2. Alternativamente, usa [`docs/lidarr-artists-import.txt`](./lidarr-artists-import.txt) — un nombre por línea — como checklist para altas masivas o para alimentar un script propio contra la API de Lidarr (`POST /api/v1/artist/lookup`).
3. Los nombres se dejan en su forma más reconocible en MusicBrainz para maximizar el match automático.
